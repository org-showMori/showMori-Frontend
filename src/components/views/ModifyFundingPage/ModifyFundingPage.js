...
function ModifyFundingPage(props) {
  ...
  useEffect(() => {
    dispatch(beforeModifyFunding(postId)).then((response) => {
      ...
      response.payload.reward_list.map((e) => {
        return printRewardList(e.reward_money, e.reward);
      });
    });
  }, []);
  const chkFormValidate = () => {
    ...
    const checkValidateDate = (start, last, deadline) => {
    ...
    checkValidateDate(StartDate, LastDate, DeadLine);
  };
